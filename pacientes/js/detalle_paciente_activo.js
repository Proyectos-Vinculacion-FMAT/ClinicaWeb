document.addEventListener('DOMContentLoaded', function() {
    ...
    const handleArchivar = () => {
        if(confirm('¿Estás seguro que deseas archivar este expediente? El paciente pasará a estado "Archivado"')) {
            ...
            mostrarNotificacion('success', 'Expediente archivado correctamente (simulado)');
    ...
    const handleGuardarInfo = () => {
        if(confirm('¿Guardar los cambios realizados?')) {
            ...
            inputsInfo.forEach(input => {
                ...
                if(input.name === 'fecha_registro') paciente.fechaRegistro = input.value;
                if(input.name === 'proxima_sesion') paciente.proximaSesion = input.value;
            });
            
            // Simulamos el guardado exitoso
            mostrarNotificacion('success', 'Cambios guardados correctamente (simulado)');
    ...
    const cargarDatosPaciente = () => {
        ...
        document.querySelector('[name="servicio"]').value = paciente.servicio || 'psicodiagnostico';
        document.querySelector('[name="terapeuta"]').value = paciente.terapeutaId || '1';
